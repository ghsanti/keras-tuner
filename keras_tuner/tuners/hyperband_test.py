...
def build_model(hp):
    ...
    for i in range(hp.Int("layers", 1, 3)):
        model.add(
            keras.layers.Dense(hp.Int(f"units{i!s}", 1, 5), activation="relu")
        )

        model.add(
            keras.layers.Lambda(lambda x: x + hp.Float(f"bias{i!s}", -1, 1))
        )
...
def test_hyperband_oracle_one_sweep_parallel(tmp_path):
    ...
    round0_trials = []
    for i in range(10):
        t = oracle.create_trial(f"tuner{i!s}")
...
def test_hyperband_load_weights(tmp_path):
    ...
    assert all(
        ...
        for new_weight, best_old_weight in zip(
            new_model_weights, best_model_round_0_weights, strict=False
        )
    )
