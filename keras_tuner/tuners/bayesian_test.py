...
def build_model(hp):
    ...
    for i in range(3):
        model.add(
            keras.layers.Dense(
                units=hp.Int(f"units_{i!s}", 2, 4, 2), activation="relu"
            )
        )
...
def test_distributed_optimization(tmp_path):
    ...
    def evaluate(hp):
        ...
        return abs(hp["a"] - 4) - hp["b"] + 0.1 * abs(3 + math.log10(hp["c"]))
    ...
    for _ in range(10):
        ...
        for i in range(tuners):
            trial = oracle.create_trial(f"tuner_{i!s}")
            trials.append(trial)
        for trial in trials:
            oracle.update_trial(
    ...
    log_best_c = math.log10(best_hps["c"])
