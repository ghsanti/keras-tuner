...
import keras_tuner
from keras_tuner.distribute import oracle_chief, oracle_client
from keras_tuner.test_utils import mock_distribute
from keras_tuner.tuners import randomsearch


def test_get_space(tmp_path: str):
    """Test the parameters drawn.
    1. Pytest will execute files starting with `test_` as part of the test run.
    2. Then the mockdistribute will run the _test_get_space().
    3. The passes a HP and Objective to the Oracle.
    4. The oracle selects init HP-values which are tested.
    """
...
def test_update_space(tmp_path: str):
    """Test the update of the parameters."""
    ...
    def _test_update_space():
        ...
        if "chief" in tuner_id:
            ...
            retrieved_hps = client.get_space()
            assert len(retrieved_hps.space) == 2  # b:Choice, a:Int
...
def test_create_trial(tmp_path: str):
...
def test_update_trial(tmp_path):
    def _test_update_trial():
        ...
        if "chief" in tuner_id:
            ...
            client.update_trial(trial_id, {"score": 1})
            updated_trial = client.get_trial(trial_id)
            assert updated_trial.metrics.get_history("score") == [[1.0]]
