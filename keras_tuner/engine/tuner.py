...
if TYPE_CHECKING:
    ...
    Oracle = Any
    _SomeObjective = Any
    _kerasMetric = Any  # noqa: N816
    _kerasOptimizer = Any  # noqa: N816
    _kerasLoss = Any  # noqa: N816
    _Model = Any
...
class Tuner(base_tuner.BaseTuner):
    ...
    tuners can be created byglob_patternbclassing the class.
    ...
    def __init__(
        ...
        optimizer: _kerasOptimizer | None = None,
        loss: _kerasLoss | None = None,
        metrics: _kerasMetric | None = None,
    ...
    def _override_compile_args(self, model: _Model) -> None:
        with maybe_distribute(self.distribution_strategy):
    ...
    def run_trial(self, trial: Trial, *args, **kwargs):
        ...
        model_checkpoint = tuner_utils.SaveBestEpoch(
            objective=cast(_SomeObjective, self.oracle.objective),
            keras_path=self._get_checkpoint_fname(trial.trial_id),
        )
    ...
    def load_model(self, trial: Trial):
        ...
        with maybe_distribute(self.distribution_strategy):
            model.load(self._get_checkpoint_fname(trial.trial_id))
        return model

    def on_batch_begin(self, trial: Trial, model: _Model, batch: int, logs):
    ...
    def on_batch_end(self, trial: Trial, model: _Model, batch: int, logs=None):
    ...
    def on_epoch_begin(
        self, trial: Trial, model: _Model, epoch: int, logs=None
    ):
    ...
    def on_epoch_end(self, trial: Trial, model: _Model, epoch: int, logs=None):
    ...
    def _deepcopy_callbacks(self, callbacks):
        try:
            ...
            msg = f"""All callbacks used during a search
                should be deep-copyable (since they are
             reused across trials).
                It is not possible to do `copy.deepcopy({callbacks})`"""
    ...
    def _get_tensorboard_dir(self, logdir: str, trial_id: str, execution: int):
        return Path(str(logdir), str(trial_id), f"execution{execution!s}")

    def _get_checkpoint_fname(self, trial_id: str) -> Path:
        return Path(
...
def maybe_distribute(distribution_strategy):
    """Distribute if distribution_strategy is set."""
