...
def build_model(hp):
    ...
    for i in range(hp.Int("num_layers", 1, 4)):
        x = keras.layers.Dense(
            units=hp.Int(f"units_{i!s}", 5, 9, 1, default=6),
            activation="relu",
        )(x)
...
def test_fit_return_string(tmp_path):
    ...
    with pytest.raises(
        ...
        match="Expected the return value of HyperModel.fit()",
    ):
        tuner.search()
...
def test_run_trial_return_string(tmp_path):
    ...
    with pytest.raises(TypeError, match="Expected the return value of"):
        tuner.search()
...
def test_no_objective_fit_return_not_float(tmp_path):
    ...
    with pytest.raises(
        TypeError, match="Expected the return value of HyperModel.fit()"
    ):
        tuner.search()
...
def test_no_objective_run_trial_return_not_float(tmp_path):
    ...
    with pytest.raises(
        TypeError, match="Expected the return value of Tuner.run_trial()"
    ):
        tuner.search()
...
def test_static_space(tmp_path):
    def build_model_static(hp):
        ...
        for i in range(hp.get("num_layers")):
            x = keras.layers.Dense(
                units=hp.get(f"units_{i!s}"), activation="relu"
            )(x)
        outputs = keras.layers.Dense(NUM_CLASSES, activation="softmax")(x)
        model = keras.Model(inputs, outputs)
        model.compile(
