...
class HyperParameters:
    ...
    def merge(self, hps, overwrite=True):
        """Merge hyperparameters into this object.
    ...
    def from_proto(cls, proto):
        ...
        space = []
        if isinstance(proto, protos.HyperParameters.Values):
    ...
    def to_proto(self):
        ...
        for name, value in self.values.items():
            if isinstance(value, bool):
                val = protos.Value(boolean_value=value)
            elif isinstance(value, float):
                val = protos.Value(float_value=value)
            elif isinstance(value, int):
                val = protos.Value(int_value=value)
        ...
        return protos.HyperParameters(
            space=protos.HyperParameters.Space(
            ...
            values=protos.HyperParameters.Values(values=values),
        )
