...
def test_choice():
    choice = Choice("choice", [1, 2, 3], default=2)
    choice = Choice.from_config(choice.get_config())
...
def test_choice_repr():
    assert repr(Choice("a", [1, 2, 3])) == repr(Choice("a", [1, 2, 3]))


def test_choice_none_as_default():
    hp = Choice("a", [1, 2], default=None)
    assert hp.default == 1


def test_choice_default_not_none():
    hp = Choice("a", [1, 2], default=2)
    assert hp.default == 2


def test_choice_proto():
    hp = Choice("a", [2.3, 4.5, 6.3], ordered=True)
    proto = hp.to_proto()
    assert proto.name == "a"
    ...
    new_hp = Choice.from_proto(proto)
    ...
    # Test int values.
    int_choice = Choice("b", [1, 2, 3], ordered=False, default=2)
    new_int_choice = Choice.from_proto(int_choice.to_proto())
    assert int_choice.get_config() == new_int_choice.get_config()

    # Test float values.
    float_choice = Choice("b", [0.5, 2.5, 4.0], ordered=False, default=2.5)
    new_float_choice = Choice.from_proto(float_choice.to_proto())
...
def test_choice_values_property():
    assert list(Choice("choice", [0, 1, 2]).values) == [0, 1, 2]
