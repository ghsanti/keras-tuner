...
def test_float():
    ...
    linear = Float.from_config(linear.get_config())
    ...
    linear = Float("linear", min_value=0.5, max_value=6.5, default=2.0)
    linear = Float.from_config(linear.get_config())
...
def test_log_sampling_random_state():
    f = Float("f", 1e-3, 1e3, sampling="log")
    rand_sample = f.random_sample()
...
def test_reverse_log_sampling_random_state():
    f = Float("f", 1e-3, 1e3, sampling="reverse_log")
    rand_sample = f.random_sample()
    assert rand_sample >= f.min_value
    assert rand_sample <= f.max_value
...
def test_float_values_property_without_step():
    assert len(list(Float("float", 2, 4).values)) == 10
    assert len(list(Float("float", 2, 20).values)) == 10
    assert len(list(Float("float", 2, 1024, sampling="log").values)) == 10
