...
import keras
from . import serialize, deserialize, HyperParameters
...
def test_to_proto_unrecognized_hp_type():
    ...
    with pytest.raises(TypeError, match="Unrecognized HP"):
...
def test_hyperparameters_proto():
    ...
    new_hps = HyperParameters.from_proto(hps.to_proto())
    assert _sort_space(hps) == _sort_space(new_hps)
    assert hps.values == new_hps.values


def test_hyperparameters_values_proto():
    values = protos.HyperParameters.Values(
        values={
            "a": protos.Value(int_value=1),
            "b": protos.Value(float_value=2.0),
            "c": protos.Value(string_value="3"),
        }
    )

    # When only values are provided, each param is created as `Fixed`.
    hps = HyperParameters.from_proto(values)
