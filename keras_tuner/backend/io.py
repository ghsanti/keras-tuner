...
def glob(glob_pattern: str) -> list[str]:
...
def _get_base_dirpath(strategy):
    ...
    return f"workertemp_{task_id!s}"


def _is_temp_dir(dirpath: Path, strategy) -> bool:
    return str(dirpath).endswith(_get_base_dirpath(strategy))


def _get_temp_dir(dirpath: Path, strategy) -> Path:
    if tf is None:
        msg = "Tensorflow is not imported but it's needed."
        raise RuntimeError(msg)
    if _is_temp_dir(dirpath, strategy):
        temp_dir = dirpath
    else:
        temp_dir = Path(dirpath, _get_base_dirpath(strategy))
    tf.io.gfile.makedirs(str(temp_dir))
    return temp_dir


def write_dirpath(dirpath: Path, strategy) -> Path | None:
    """Return the writing dir that should be used to save file distributedly.
...
def remove_temp_dirpath(dirpath: Path, strategy) -> None:
    """Remove the temp path after writing is finished.
...
def write_filepath(filepath: Path, strategy) -> Path | None:
    """Return the writing file path to be used to save file distributedly.
    ...
    tmp_path = write_dirpath(dirpath, strategy)
    if tmp_path is not None:
        return tmp_path / base
    return None


def remove_temp_dir_with_filepath(filepath: Path, strategy) -> None:
    """Remove the temp path for file after writing is finished.
    ...
    remove_temp_dirpath(Path(filepath).parent, strategy)
